...
#' Default xlim for Barplot
#'
#' @param position vector of x-positions of bars
#' @param barGroupWidth bargroup widths. Default: 1
#' @return vector of numeric of length two
hsBpDefaultXlim <- function(position, barGroupWidth = 1)
{
...
#' xlim for Barplot
#'
#' Depending on the argument values that are passed to \code{barplot()} a good
#' vector to be used as xlim is returned.
#'
#' @param myData corresponding to "height" in \code{\link{barplot}}
#' @param myWidth corresponding to "width" in \code{\link{barplot}}
#' @param mySpace corresponding to "space" in \code{\link{barplot}}
#' @param myBeside corresponding to "beside" in \code{\link{barplot}}
#' @return vector of numeric of length two
hsBpGetXlim <- function(myData, myWidth = 1, mySpace = NULL, myBeside = FALSE)
{
...
#' Width of Bargroup
#'
#' The widths represent the widths of the bars in one group. It is calculated
#' as: sum(myWidth) + (myBarsPerGroup - 1) * myInnerSpace
#'
#' @param myWidth widths of the bars in one group
#' @param myBarsPerGroup number of bars per group
#' @param myInnerSpace inner space between bars in a bar group
#' @return vector of numeric of length one
hsBpBargroupWidth <- function(myWidth, myBarsPerGroup, myInnerSpace)
{
...
#' Number of Bar Groups and Bars Per Group Produced by barplot()
#'
#' Number of bar groups (N) and bars per group (n) produced by barplot(),
#' depending on given data and parameter "beside"
#'
#' @param myData as passed in \code{height} to \code{\link{barplot}}
#' @param myBeside as passed in \code{beside} to \code{\link{barplot}}
#' @return Returns tupel containing N = number of bar groups and n = number of
#'   bars per group
#'
hsBpNumBars <- function(myData, myBeside)
{
  if (!is.matrix(myData)) {
    return(c(N = length(myData), n = 1))
  }
...
#'
#' @param myWidth Bar widths. If bars are to be stacked (isp = NULL, see below)
...
#' @param myXlim Vector containing minimum and maximum x-value to be shown in
#'   the plot.
#' @param myBeside If TRUE, bars of a bar group are not stacked but arranged
...
#' @param barsPerGroup Number of bars per group. If this parameter is greater
#'   than 1, it means that bars are to be arranged side by side instead of
#'   stacked.
#' @param innerSpace "Inner" space between bars of the same group (only used if
#'   bpg > 1).
#' @param dbg whether to print debug messages or not
#' @return Returns a space vector as needed to be passed to the barplot function
#'
hsBpSpace <- function(
  myWidth, position, myXlim = NULL, myBeside = FALSE, barsPerGroup = 1,
  innerSpace = 0, dbg = FALSE
) {
...
#' barplot() with X-Position Support
#'
#' Extended version of R barplot function, enabling x positioning of the bars.
#'
#' @param myHeight Vector or matrix containing the bar heights, see R's barplot
#'   function.
#' @param myWidth Bar widths. If bars are to be stacked (isp = NULL, see below)
#'   w represents the widths of the stacked bars. If bars shall be plotted side
#'   by side instead of stacked (isp >= 0) w represents the widths of the bars
#'   within one and the same bar group. Corresponding bars of different groups
#'   will always have the same width.
#' @param myPosition Vector containing the x-positions of the bars/bar groups.
#' @param myXlim Vector containing minimum and maximum x-value to be shown in
#'   the plot.
#' @param myBeside If TRUE, bars within a bar group are arranged side by side
#'   instead of stacked.
#' @param myReverse If TRUE, the bars in the plot will reverted, i.e. they will
#'   be arranged according to decreasing x-values.
#' @param myInnerSpace "Inner" space between bars of the same bar group (only
#'   relevant if y is a matrix and not a vector). If NULL, bars of the same bar
#'   group will be stacked
#' @param myAxis whether to draw an axis or not
#' @param dbg whether to print debug messages or not
#' @param myYlim ylim vector
#' @param myValLabs whether to put value labels on top of the bars
#' @param \dots further arguments passed to \code{\link{barplot}}
#' @export
#' @examples
...
)
{
  ...
  if (myAxis) {
    if (!is.null(myXlim)) {
      ...
      graphics::axis(side = 3, xaxp=c(0, ma, ma))
    }
    else {
...
#'
#' @param myHeight heights of bars
#' @param myPosition x-positions of bars
#' @param myWidth widths of bars
#' @param mySpace space between bars in a bar group
#' @param myYmax y limit. Used to determine the distance between top of bar and
#'   labels on top
hsPutValueLabels <- function(myHeight, myPosition, myWidth, mySpace, myYmax)
{
  i <- 1
...
#' barplot() with X-Position Support (2)
#'
#' Extended version of R barplot function, enabling x positioning of the bars.
#'
#' @param myHeight Vector or matrix containing the bar heights, see R's barplot
...
#'   group will be stacked
#' @param myValLabs whether to put value labels on top of the bars
#' @param dx x offset for text labels. Default: 0
#' @param dy y offset for text labels. Default: 0
#' @param dbg whether to print debug messages or not
#' @param \dots further arguments passed to \code{\link{barplot}}
#' @export
hsBarplot2 <- function
...
)
{
  ...
  if (myValLabs) {
    ...
    graphics::text(
      x = myx + dx, y = myy + dy, labels = myv, adj = c(0, 0.5), cex = 0.7
    )
  }
}
